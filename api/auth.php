...
class AuthAPI
{
    ...
    private function login()
    {
        ...
        try {
            ...
            $updateStmt = $this->conn->prepare("UPDATE usuarios SET ultimo_acceso = NOW() WHERE id = :id");
    ...
    private function register()
    {
        ...
        try {
            ...
            $checkStmt = $this->conn->prepare("SELECT id FROM usuarios WHERE username = :username OR email = :email");
            ...
            // Encriptar contraseña en password_hash
            $password_hash = password_hash($password, PASSWORD_DEFAULT);

            $insertStmt = $this->conn->prepare(
                "INSERT INTO usuarios (username, email, password_hash, nombre, apellido, telefono)
            ...
            if ($insertStmt->execute()) {
                ...
                $getUserStmt = $this->conn->prepare(
                    "SELECT id, username, email, nombre, apellido, rol FROM usuarios WHERE id = :id"
                );
            ...
            return $this->sendResponse(false, 'Error en la base de datos: ' . $e->getMessage(), null, 500);
        }
    }
    ...
    private function getProfile()
    {
        ...
        try {
            $stmt = $this->conn->prepare(
                "SELECT id, username, email, nombre, apellido, telefono, rol, fecha_registro, ultimo_acceso
            ...
            return $this->sendResponse(true, 'Perfil obtenido', $stmt->fetch());
    ...
    private function updateProfile()
    {
        ...
        try {
            $upd = $this->conn->prepare(
                "UPDATE usuarios SET nombre = :n, apellido = :a, telefono = :t WHERE id = :id"
            );
    ...
    private function requestPasswordReset()
    {
        ...
        try {
            ...
            $upd = $this->conn->prepare(
                "UPDATE usuarios SET password_reset_token = :t, password_reset_expires = :e WHERE id = :id"
            );
            ...
            $app_path = rtrim(dirname(dirname($_SERVER['REQUEST_URI'])), '/');
            $reset_link = $base_url . $app_path . '/reset.html?token=' . $token;
    ...
    private function resetPassword()
    {
        ...
        try {
            $stmt = $this->conn->prepare(
                "SELECT id, password_reset_expires FROM usuarios WHERE password_reset_token = :t"
            );
            ...
            $upd = $this->conn->prepare(
                "UPDATE usuarios SET password_hash = :p, password_reset_token = NULL, password_reset_expires = NULL WHERE id = :id"
            );
    ...
    private function sendVerification()
    {
        ...
        try {
            $stmt = $this->conn->prepare(
                "SELECT id, email_verified FROM usuarios WHERE email = :email AND activo = 1"
            );
            $stmt->bindParam(':email', $email);
            $stmt->execute();
            ...
            $token = bin2hex(random_bytes(16));
            $upd = $this->conn->prepare(
                "UPDATE usuarios SET email_verification_token = :t WHERE id = :id"
            );
            $upd->bindParam(':t', $token);
            $upd->bindParam(':id', $user['id']);
            $upd->execute();
            ...
            $base_url = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'];
            $app_path = rtrim(dirname(dirname($_SERVER['REQUEST_URI'])), '/');
            $verify_link = $base_url . $app_path . '/verify.html?token=' . $token;
    ...
    private function verifyEmail()
    {
        ...
        try {
            $stmt = $this->conn->prepare(
                "SELECT id FROM usuarios WHERE email_verification_token = :t"
            );
            $stmt->bindParam(':t', $token);
            $stmt->execute();
            ...
            $user = $stmt->fetch();
            $upd = $this->conn->prepare(
                "UPDATE usuarios SET email_verified = 1, email_verification_token = NULL WHERE id = :id"
            );
            $upd->bindParam(':id', $user['id']);
            $upd->execute();
